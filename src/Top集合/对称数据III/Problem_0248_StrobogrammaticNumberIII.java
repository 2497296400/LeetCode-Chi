...
public class Problem_0248_StrobogrammaticNumberIII {
    ...
    public int simple(String left, String right) {
        this.low = left;
        this.high = right;
        cnt = 0;
        solve("");
    ...
    void solve(String num) {
        if((num.length() > high.length()) ||                 // 递归边界：若num的长度 > high的长度
                (num.length()==high.length() && num.compareTo(high)>0))      // 或长度相等,但num比high大 (string可用>直接比较)
            return;

        // 若num在[low,high]内, 就计数
        if(!(num.length()>1 && num.charAt(0)=='0') &&             // num开头不是0
                ((num.length() > low.length()) ||
                        (num.length()==low.length() && num.compareTo(low)>=0))        // 且num>=low (string可用>直接比较)
        )
            ++cnt;
